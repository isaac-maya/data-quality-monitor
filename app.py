...
from branding import apply_branding, footer, hero, section
...
hero(
    ...
    tagline="Drop in any CSV. Get a six-dimension data quality verdict in 3 seconds.",
    kicker="Isaac Maya · Demo Collection · Data Quality",
    badges=["Data Quality Engineer", "Analytics Engineer", "QA Lead"],
)
...
Six dimensions get checked: **Completeness, Uniqueness, Validity, Anomaly, Lineage, Timeliness.**
Each finding ships with a remediation written in plain English. The verdict at the top tells you
whether the data is safe to publish or needs review *before* it lands in front of an executive.

Bring your own CSV. The sample is just here so you don't have to.
"""
)
...
- ✅ Runs in the Streamlit container; your CSV doesn't leave the server beyond this session
"""
    )
...
section("Try it")
st.markdown(
    "👈 Upload a CSV, or roll with the bundled sample. Required columns: "
    "`record_id`, `customer_id`, `transaction_date`, `region`, `order_amount`, `source_system`."
)
...
c4.metric("Impacted records", impacted)
...
if findings:
    ...
    if not filtered.empty:
        ...
        chosen = st.selectbox(
            "Inspect a record — see the original row with violating fields called out",
        ...
        if chosen != "(pick one)":
            ...
            if not original.empty:
                ...
                st.info(f"Record `{chosen}` not in the source dataframe (may have been a missing-key finding).")
else:
    st.success("No findings — your data passed all six checks.")

# ---------- How to test it ----------
section("How to test it — guided tour")
st.markdown(
    """
**Step 1 — Read the verdict.** Load the bundled sample. You should see a 🛑 **DO NOT PUBLISH** banner.
Note which dimension wedge in the donut is largest — that's where the trouble lives.
...
the verdict should flip to ✅ **SAFE TO PUBLISH**. Then upload one with a duplicate `record_id` value —
watch the verdict cross the high-severity threshold in real time.

**Step 5 — Try your own data.** The required columns are listed above. Most operational/transactional
CSVs fit the shape. The point isn't that this scanner replaces your DQ tooling — it's that the six
dimensions are a universal frame you can apply to any dataset.
"""
)

# ---------- Hiring takeaway ----------
section("What this proves about me")
st.markdown(
    """
...
**For QA Lead roles:** I make threshold logic visible — anomaly detection is `median × 8`, not a magic
number. A scanner you can argue with is a scanner you can trust.
"""
)
